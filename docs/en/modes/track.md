...
| `match_thresh`      | `0.0-1.0`                                     | Threshold for matching tracks. For IoU: cost threshold where lower means stricter matching. For L2: normalized distance threshold as a fraction of the image diagonal (e.g., `0.3` = 30% of diagonal). |
...
| `distance_metric`   | `iou`, `l2`                                   | Distance metric for track association. `iou` uses Intersection over Union cost. `l2` uses Euclidean distance between bbox centers normalized by image diagonal, making it resolution-independent. |
| `kalman_weight`     | `0.0-1.0`                                     | Kalman filter weight for motion prediction. `1.0` = full Kalman prediction (default), `0.0` = raw detection position only. Lower values reduce prediction overshoot for objects with erratic motion (e.g., balls, drones). Lost tracks always use full Kalman prediction to continue along their trajectory. |
